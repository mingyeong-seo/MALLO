...
- 2026-08-20: FE QA 보고("GET /records 목록에서 같은 photo_id가 actions 개수만큼 photos[]에 중복 반환") 대응 — 원인은 `RecoveryRecordRepository.findBySessionIdOrderByElapsedDayAsc`/`findBySessionIdAndElapsedDay`가 `@EntityGraph(attributePaths = {"photoRecords", "actions"})`로 두 컬렉션을 한 쿼리에 fetch join하는데, SQL 결과가 (사진 수 x 행동 수) 카테시안곱으로 나오는 것. `actions`는 `@OrderColumn` 인덱스 리스트라 같은 인덱스 위치에 덮어써져서 중복이 안 보였지만, `photoRecords`는 `List`(Hibernate bag)라 곱해진 행 수만큼 그대로 append됨 — 행동 2개짜리 기록에 사진 1장 붙이면 photos[]에 그 사진이 2번 나오는 식. 고친 건 `RecoveryRecord.photoRecords` 타입을 `List` → `Set`(`LinkedHashSet`)으로 변경 — Hibernate가 동일 id 엔티티를 중복 추가하지 않아서 기존처럼 쿼리 1번 유지하면서 중복만 사라짐(`@OrderBy("id asc")`는 Set에도 그대로 적용돼 순서 유지). `RecoveryRecordService`/`attachPhotos` 등 `List` 전용 메서드를 쓰는 곳 없어서 필드 타입 변경만으로 끝남. 재현 테스트(행동 2개+사진 1장 → 고치기 전엔 `[8L, 8L]`로 실제 중복 재현 확인 후 되돌려서 재검증) 추가. 실제 MySQL 확인 전(로컬 DataJpaTest만) — FCM/사진 흐름 자체는 FE가 이미 정상 확인함, 이 중복만 별개 이슈였음. 테스트 206개(전체)
